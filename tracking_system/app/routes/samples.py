...
from app.schemas.pydantic_models import SamplesToMakeResponse, QCResultsInput, SamplesToShipResponse, SamplesShippedInput
from app.services.sample_service import get_samples_to_process, log_qc_results, get_samples_to_ship, record_samples_shipped
...
async def log_qc_results_route(qc_results_input: QCResultsInput, session: AsyncSession = Depends(get_session)):
...
async def record_samples_shipped_route(samples_shipped_input: SamplesShippedInput, session: AsyncSession = Depends(get_session)):
